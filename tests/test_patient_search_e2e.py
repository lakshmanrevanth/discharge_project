...
def test_pending_clear_happens_before_search_widget(monkeypatch):
    """One search-bar pass: pending clear → query emptied before the widget mounts.

    The search moved out of the sidebar into the main column; the rail now only
    renders navigation, so this asserts against ``_search_bar``.
    """
    import dashboard.app as app_mod
    ...
    # Import path used inside _search_bar
