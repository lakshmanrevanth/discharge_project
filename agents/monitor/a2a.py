...
# Any digit length — the Host embeds the patient id in the scan request text.
...
class MonitorAgentExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            ...
            patient_id = match.group(0).upper() if match else "scan"
            with case_trace(patient_id, existing=incoming_tid, host_name="Monitor Agent"):
