...
async def report_node(state: ValidatorState) -> dict:
    ...
    from shared.tracing.langfuse import get_current_trace_id, record_mcp_tool

    url = _primary_mcp_url()
    errors = list(state.get("errors", []))
    normalization = state.get("normalization") or {}
    # The Primary MCP process cannot see our trace ContextVar — hand it the case
    # trace id explicitly so audit_trail.langfuse_url resolves to a real trace.
    params = {
